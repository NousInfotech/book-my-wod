...
class _EditSessionScreenState extends State<EditSessionScreen> {
  ...
  late TextEditingController _entryLimitController;
  late TextEditingController _catagoryDescriptionController;
  TimeOfDay? startTime;
  TimeOfDay? endTime;
  int seatLimit = 0;
  List<DateTime> _selectedDates = [];
  String _sessionRepeat = 'none';
  String _imageUrl = '';
  ...
  void initState() {
    ...
    _sessionNameController = TextEditingController(
        text: widget.sessionModel != null ? widget.sessionModel['name'] : '');
    _entryLimitController = TextEditingController(
        text: widget.sessionModel != null ? widget.sessionModel['entryLimit']?.toString() ?? '10' : '10');
    _catagoryDescriptionController = TextEditingController(
        text: widget.sessionModel != null ? widget.sessionModel['description'] ?? '' : '');
        
    // Initialize times
    if (widget.sessionModel != null) {
      startTime = _parseTime(widget.sessionModel['startTime'] ?? '00:00');
      endTime = _parseTime(widget.sessionModel['endTime'] ?? '01:00');
  ...
  Future<void> _saveChanges() async {
    ...
    if (widget.sessionModel == null) {
      ...
      final session = SessionModel(
        ...
        timeSlots: timeSlots.map((e) => Map<String, String>.from(e)).toList(),
        days: _selectedDates
            .map((d) => DateFormat('EEEE, M/d/y').format(d))
            .toList(),
        sessionRepeat: _sessionRepeat == 'none' ? false : true,
