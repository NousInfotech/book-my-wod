...
// Modify this method in your SupabaseDb class
Future<List<AdminModel>> getAdmins() async {
  try {
    // First check if the table exists
    final tableExists = await _checkIfTableExists('admins');
    if (!tableExists) {
      print('Table "admins" does not exist yet. Returning empty list.');
      return [];
    }
    ...
    return (response as List)
        .map((admin) => AdminModel.fromMap(admin))
        .toList();
  } catch (e) {
...
Future<bool> _checkIfTableExists(String tableName) async {
  try {
    // This is a way to check if a table exists without throwing exceptions
    // It tries to get 0 rows from the table - if the table exists, it succeeds
    ...
    // If it's another type of error, assume the table exists
    return true;
  }
}
